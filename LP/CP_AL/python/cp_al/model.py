...
class _ParamsView:
    ...
    def __getattr__(self, name: str):
        key = CPAL._PARAM_ALIAS.get(name, name)
...
class Model:
    ...
    def __init__(
        ...
        - model_sense: CPAL.MINIMIZE or CPAL.MAXIMIZE.
        ...
        # sense
        self.ModelSense = CPAL.MINIMIZE
        # always start from backend defaults CPAL params
    ...
    def optimize(self):
        """
        Solve the linear programming problem using the CP_AL solver.
        """
        # clear cached solution
        self._clear_solution_cache()
        # check model sense
        if self.ModelSense not in (CPAL.MINIMIZE, CPAL.MAXIMIZE):
            raise ValueError("model_sense must be CPAL.MINIMIZE or CPAL.MAXIMIZE")
        # determine sign
        sign = 1 if self.ModelSense == CPAL.MINIMIZE else -1

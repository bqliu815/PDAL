...
static void compute_trajectory_diagnostics(const cp_al_solver_state_t *state,
                                           double *sigma,
  ...
  *dual_step = NAN;
  ...
  if (state->use_al_qp) {
    ...
    *primal_step = state->step_size;
    *dual_step = state->sigma;
  } else if (state->primal_weight > 0.0) {

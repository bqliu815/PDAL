...
class _ParamsView:
    ...
    def __getattr__(self, name: str):
        key = FA_CP._PARAM_ALIAS.get(name, name)
...
class Model:
    ...
    def __init__(
        ...
        - model_sense: FA_CP.MINIMIZE or FA_CP.MAXIMIZE.
        ...
        # sense
        self.ModelSense = FA_CP.MINIMIZE
        # always start from backend defaults FA_CP params
    ...
    def optimize(self):
        """
        Solve the linear programming problem using the FA_CP solver.
        """
        # clear cached solution
        self._clear_solution_cache()
        # check model sense
        if self.ModelSense not in (FA_CP.MINIMIZE, FA_CP.MAXIMIZE):
            raise ValueError("model_sense must be FA_CP.MINIMIZE or FA_CP.MAXIMIZE")
        # determine sign
        sign = 1 if self.ModelSense == FA_CP.MINIMIZE else -1

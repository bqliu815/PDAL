...
static void initialize_quadratic_obj_term(fa_cp_solver_state_t *state,
                                          const qp_problem_t *problem) {
  state->quadratic_objective_term = (quadratic_objective_term_t *)safe_calloc(
      1, sizeof(quadratic_objective_term_t));
...
static void initialize_inner_solver(fa_cp_solver_state_t *state) {
  ...
  bool may_use_al_qp = state->al_qp_available;
  state->inner_solver->has_inner_loop = state->use_al_qp || may_use_al_qp;
  if (may_use_al_qp ||
      !(state->quadratic_objective_term->quad_obj_type == FA_CP_NON_Q ||
...
initialize_solver_state(const fa_cp_parameters_t *params,
  ...
  state->al_qp_available = params->use_al_qp;
  state->use_al_qp = params->use_al_qp;
  state->al_sigma = params->al_sigma;
  state->best_al_sigma = params->al_sigma;
  state->best_sigma_score = INFINITY;
  state->last_sigma_score = INFINITY;
  state->last_sigma_primal_residual = INFINITY;
  state->last_sigma_dual_residual = INFINITY;
  state->last_sigma_gap = INFINITY;
  state->sigma_update_count = 0;
  state->sigma_search_direction = 1;
  state->sigma_update_rule = params->sigma_update_rule;
  ...
  state->restart_current_count = 0;
  state->restart_halpern_count = 0;
  state->step_size = 0.0;
  ...
  if (state->al_qp_available &&
      params->al_sigma <= params->sigma_zero_threshold) {

...
void set_default_parameters(fa_cp_parameters_t *params) {
  ...
  params->al_sigma = 0.0;
  params->sigma_update_rule = FA_CP_SIGMA_UPDATE_GUARDED;
  params->sigma_update_gain = 0.5;
  params->sigma_clip_low = 0.6;
  params->sigma_clip_high = 1.7;
  params->sigma_min = 1e-8;
  params->sigma_max = 10.0;
  params->sigma_rollback_tol = 1.25;
  params->sigma_zero_threshold = 1e-8;
  params->sigma_activation_threshold = 2.0;
  params->sigma_normal_motion_threshold = 1e-10;
  params->sigma_activation_value = 0.1;
  params->verbose = 1;
...
void print_initial_info(const fa_cp_parameters_t *params,
  ...
  printf("                                   FA_CP                            "
         "    "
         "                  \n");
  printf("      A GPU-Accelerated Augmented Fixed-Point/Correction Solver      "
         "                  \n");
  ...
  if (params->sigma_update_rule != default_params.sigma_update_rule) {
    printf("  %-18s : %s\n", "sigma_update",
           sigma_update_rule_to_string(params->sigma_update_rule));
  }
  PRINT_DIFF_DBL("sigma_update_gain", params->sigma_update_gain,
                 default_params.sigma_update_gain);
  PRINT_DIFF_DBL("sigma_clip_low", params->sigma_clip_low,
                 default_params.sigma_clip_low);
  PRINT_DIFF_DBL("sigma_clip_high", params->sigma_clip_high,
                 default_params.sigma_clip_high);
  PRINT_DIFF_DBL("sigma_min", params->sigma_min, default_params.sigma_min);
  PRINT_DIFF_DBL("sigma_max", params->sigma_max, default_params.sigma_max);
  PRINT_DIFF_DBL("sigma_rollback_tol", params->sigma_rollback_tol,
                 default_params.sigma_rollback_tol);
  PRINT_DIFF_DBL("sigma_zero_threshold", params->sigma_zero_threshold,
                 default_params.sigma_zero_threshold);
  PRINT_DIFF_DBL("sigma_activation_threshold",
                 params->sigma_activation_threshold,
                 default_params.sigma_activation_threshold);
  PRINT_DIFF_DBL("sigma_normal_motion_threshold",
                 params->sigma_normal_motion_threshold,
                 default_params.sigma_normal_motion_threshold);
  PRINT_DIFF_DBL("sigma_activation_value", params->sigma_activation_value,
                 default_params.sigma_activation_value);

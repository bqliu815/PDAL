...
static void initialize_trajectory_csv(const char *trajectory_csv) {
  ...
  fprintf(outfile,
          ...
          "fixed_point_residual,restart,al_sigma,sigma_update_count\n");
  fclose(outfile);
}
...
static void append_trajectory_row(const char *trajectory_csv,
  ...
  fprintf(outfile, "%d,%.17e,%.17e,%.17e,%.17e,%.17e,%.17e,%d,%.17e,%d\n",
          ...
          restart_flag ? 1 : 0, state->al_sigma, state->sigma_update_count);
  fclose(outfile);
}

...
#ifndef FA_CP_VERSION
#define FA_CP_VERSION "unknown"
#endif
...
void compute_interaction_and_movement(fa_cp_solver_state_t *state,
...
bool optimality_criteria_met(const fa_cp_solver_state_t *state,
...
bool primal_infeasibility_criteria_met(const fa_cp_solver_state_t *state,
                                       double eps) {
...
bool dual_infeasibility_criteria_met(const fa_cp_solver_state_t *state,
                                     double eps) {
...
void set_default_parameters(fa_cp_parameters_t *params) {
  ...
  params->al_sigma = 0.1;
  params->adaptive_al_sigma = false;
  params->adaptive_al_sigma_mode = ADAPTIVE_AL_SIGMA_MODE_BIDIRECTIONAL;
  params->adaptive_al_sigma_rule = ADAPTIVE_AL_SIGMA_RULE_TRIGGER;
  params->al_sigma_min = 5e-3;
  params->al_sigma_max = 3e-1;
  params->al_sigma_increase_factor = 1.5;
  params->al_sigma_decrease_factor = 0.7;
  params->al_sigma_primal_trigger = 3.0;
  params->al_sigma_gap_trigger = 5.0;
  params->trajectory_csv = NULL;
  params->verbose = 1;
...
static const char *adaptive_al_sigma_mode_to_string(
    adaptive_al_sigma_mode_t mode) {
  switch (mode) {
  case ADAPTIVE_AL_SIGMA_MODE_UP_ONLY:
    return "up";
  case ADAPTIVE_AL_SIGMA_MODE_DOWN_ONLY:
    return "down";
  case ADAPTIVE_AL_SIGMA_MODE_BIDIRECTIONAL:
  default:
    return "bidir";
  }
}

static const char *adaptive_al_sigma_rule_to_string(
    adaptive_al_sigma_rule_t rule) {
  switch (rule) {
...
void print_initial_info(const fa_cp_parameters_t *params,
  ...
  fa_cp_parameters_t default_params;
  ...
  printf("                                  RHR-FA-CP                         "
         "    "
         "                  \n");
  printf("       A GPU-Accelerated FA_CP Solver for Convex QPs            "
         "                  \n");
  ...
  PRINT_DIFF_DBL("al_sigma", params->al_sigma, default_params.al_sigma);
  PRINT_DIFF_BOOL("adaptive_al_sigma", params->adaptive_al_sigma,
                  default_params.adaptive_al_sigma);
  if (params->adaptive_al_sigma_mode != default_params.adaptive_al_sigma_mode) {
    printf("  %-18s : %s\n", "al_sigma_mode",
           adaptive_al_sigma_mode_to_string(params->adaptive_al_sigma_mode));
  }
  if (params->adaptive_al_sigma_rule != default_params.adaptive_al_sigma_rule) {
    printf("  %-18s : %s\n", "al_sigma_rule",
           adaptive_al_sigma_rule_to_string(params->adaptive_al_sigma_rule));
  }
  PRINT_DIFF_DBL("al_sigma_min", params->al_sigma_min,
                 default_params.al_sigma_min);
  PRINT_DIFF_DBL("al_sigma_max", params->al_sigma_max,
                 default_params.al_sigma_max);
  PRINT_DIFF_DBL("al_sigma_up", params->al_sigma_increase_factor,
                 default_params.al_sigma_increase_factor);
  PRINT_DIFF_DBL("al_sigma_down", params->al_sigma_decrease_factor,
                 default_params.al_sigma_decrease_factor);
  PRINT_DIFF_DBL("al_sigma_primal_trigger", params->al_sigma_primal_trigger,
                 default_params.al_sigma_primal_trigger);
  PRINT_DIFF_DBL("al_sigma_gap_trigger", params->al_sigma_gap_trigger,
                 default_params.al_sigma_gap_trigger);
  PRINT_DIFF_DBL("restart_artificial",
                 params->restart_params.artificial_restart_threshold,
                 default_params.restart_params.artificial_restart_threshold);
  PRINT_DIFF_DBL("restart_sufficient",
                 params->restart_params.sufficient_reduction_for_restart,
                 default_params.restart_params.sufficient_reduction_for_restart);
  PRINT_DIFF_DBL("restart_necessary",
                 params->restart_params.necessary_reduction_for_restart,
                 default_params.restart_params.necessary_reduction_for_restart);
  PRINT_DIFF_DBL("restart_kp", params->restart_params.k_p,
                 default_params.restart_params.k_p);
  PRINT_DIFF_DBL("restart_ki", params->restart_params.k_i,
                 default_params.restart_params.k_i);
  PRINT_DIFF_DBL("restart_kd", params->restart_params.k_d,
                 default_params.restart_params.k_d);
  PRINT_DIFF_DBL("restart_i_smooth", params->restart_params.i_smooth,
                 default_params.restart_params.i_smooth);
...
void display_iteration_stats(const fa_cp_solver_state_t *state, int verbose) {
  if (verbose < 2) {
    return;
  }
...
void compute_residual(fa_cp_solver_state_t *state, norm_type_t optimality_norm) {
  cusparseDnVecSetValues(state->vec_primal_sol, state->fa_cp_primal_solution);
  ...
  if (state->problem_type == LP) {
    compute_lp_residual_kernel<<<state->num_blocks_primal_dual,
        ...
        state->fa_cp_dual_solution, state->dual_residual, state->dual_product,
    ...
    compute_qp_residual_kernel<<<state->num_blocks_primal_dual,
        ...
        state->fa_cp_primal_solution, state->constraint_lower_bound,
        state->constraint_upper_bound, state->variable_lower_bound,
        state->variable_upper_bound, state->fa_cp_dual_solution,
        ...
        state->step_size / state->primal_weight, state->num_constraints,
        state->num_variables);
  }
  ...
  double half_xQx =
      0.5 * compute_xQx(state, state->fa_cp_primal_solution,
                        state->quadratic_objective_term->primal_obj_product);

  CUBLAS_CHECK(cublasDdot(
      state->blas_handle, state->num_variables, state->objective_vector, 1,
      state->fa_cp_primal_solution, 1, &state->primal_objective_value));
  ...
  CUBLAS_CHECK(cublasDdot(state->blas_handle, state->num_variables,
                          state->dual_slack, 1, state->fa_cp_primal_solution, 1,
                          &base_dual_objective));
...
void compute_dual_feas_polish_residual(fa_cp_solver_state_t *state,
  ...
  compute_dual_feas_polish_residual_kerenl<<<state->num_blocks_primal_dual,
                                             THREADS_PER_BLOCK>>>(
      state->dual_residual, state->fa_cp_dual_solution, state->dual_product,
  ...
  double base_dual_objective;
  CUBLAS_CHECK(cublasDdot(state->blas_handle, state->num_variables,
                          state->dual_slack, 1, ori_state->fa_cp_primal_solution,
...
quad_obj_type_t detect_q_type(const CsrComponent *sparse_component,
  ...
  if (nnz_low_rank > 0) {
    if (nnz_sparse > 0) {
      return FA_CP_LOW_RANK_PLUS_SPARSE_Q;
    } else {
      return FA_CP_LOW_RANK_Q;
    }
  } else {

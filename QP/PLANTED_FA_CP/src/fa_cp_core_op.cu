...
static void reset_inner_solver_tolerance(fa_cp_solver_state_t *state) {
  if (state->inner_solver != NULL && state->inner_solver->has_inner_loop) {
    state->inner_solver->tol = state->inner_solver->initial_tol;
  }
}

static void clear_al_sigma_trial(fa_cp_solver_state_t *state) {
  state->al_sigma_trial_active = false;
  state->al_sigma_trial_previous_sigma = state->al_sigma;
  state->al_sigma_trial_reference_primal = 0.0;
  state->al_sigma_trial_reference_dual = 0.0;
  state->al_sigma_trial_reference_gap = 0.0;
  state->al_sigma_trial_start_iter = -INT32_MAX / 4;
  state->al_sigma_trial_from_gap_dual_stalled = false;
}

static bool apply_new_al_sigma(fa_cp_solver_state_t *state,
                               const fa_cp_parameters_t *params,
                               double new_sigma,
                               bool reset_inner_tol,
                               const char *reason,
                               bool clear_gap_dual_block) {
  double old_sigma = state->al_sigma;
  if (fabs(new_sigma - old_sigma) <=
      1e-12 * fmax(1.0, fabs(old_sigma))) {
    return false;
  }

  state->al_sigma = new_sigma;
  state->last_sigma_update_iter = state->total_count;
  if (reset_inner_tol) {
    reset_inner_solver_tolerance(state);
  }
  clear_al_sigma_trial(state);
  if (clear_gap_dual_block) {
    state->al_sigma_gap_dual_switch_blocked = false;
  }

  if (params->verbose >= 2) {
    printf("  adaptive_al_sigma  : %.3e -> %.3e at iter %d [%s] "
           "(rp=%.3e, rd=%.3e, rg=%.3e)\n",
           old_sigma, new_sigma, state->total_count,
           reason != NULL ? reason : "unspecified",
           fabs(state->relative_primal_residual),
           fabs(state->relative_dual_residual),
           fabs(state->relative_objective_gap));
  }
  return true;
}

static bool adaptive_sigma_mode_allows_up(
    adaptive_al_sigma_mode_t mode) {
  return mode != ADAPTIVE_AL_SIGMA_MODE_DOWN_ONLY;
}

static bool adaptive_sigma_mode_allows_down(
    adaptive_al_sigma_mode_t mode) {
  return mode != ADAPTIVE_AL_SIGMA_MODE_UP_ONLY;
}

static bool maybe_update_al_sigma_on_restart(
    fa_cp_solver_state_t *state, const fa_cp_parameters_t *params) {
  if (!params->adaptive_al_sigma || !state->use_al_qp) {
    return false;
  }
  if (state->last_sigma_update_iter == state->total_count) {
    return false;
  }

  double rel_primal = fabs(state->relative_primal_residual);
  double rel_dual = fabs(state->relative_dual_residual);
  double rel_gap = fabs(state->relative_objective_gap);
  if (!isfinite(rel_primal) || !isfinite(rel_dual) || !isfinite(rel_gap)) {
    return false;
  }

  double dual_gap = fmax(rel_dual, rel_gap);
  double primal_ref = fmax(rel_primal, 1e-12);
  double dual_gap_ref = fmax(dual_gap, 1e-12);
  double ratio = rel_primal / dual_gap_ref;
  bool primal_dominant =
      rel_primal > params->al_sigma_primal_trigger * dual_gap_ref;
  bool dual_gap_dominant =
      dual_gap > params->al_sigma_gap_trigger * primal_ref;

  double new_sigma = state->al_sigma;
  const char *reason = NULL;

  switch (params->adaptive_al_sigma_rule) {
  case ADAPTIVE_AL_SIGMA_RULE_TRIGGER:
    if (primal_dominant &&
        adaptive_sigma_mode_allows_up(params->adaptive_al_sigma_mode)) {
      new_sigma =
          fmin(params->al_sigma_max,
               state->al_sigma * params->al_sigma_increase_factor);
      reason = "restart_trigger_up";
    } else if (dual_gap_dominant &&
               adaptive_sigma_mode_allows_down(
                   params->adaptive_al_sigma_mode)) {
      new_sigma =
          fmax(params->al_sigma_min,
               state->al_sigma * params->al_sigma_decrease_factor);
      reason = "restart_trigger_down";
    }
    break;
  case ADAPTIVE_AL_SIGMA_RULE_SQRT:
    if (primal_dominant &&
        adaptive_sigma_mode_allows_up(params->adaptive_al_sigma_mode)) {
      double factor = fmin(params->al_sigma_increase_factor, sqrt(ratio));
      new_sigma = fmin(params->al_sigma_max, state->al_sigma * factor);
      reason = "restart_sqrt_up";
    } else if (dual_gap_dominant &&
               adaptive_sigma_mode_allows_down(
                   params->adaptive_al_sigma_mode)) {
      double factor =
          fmax(params->al_sigma_decrease_factor, sqrt(ratio));
      new_sigma = fmax(params->al_sigma_min, state->al_sigma * factor);
      reason = "restart_sqrt_down";
    }
    break;
  case ADAPTIVE_AL_SIGMA_RULE_BANGBANG:
    if (primal_dominant &&
        adaptive_sigma_mode_allows_up(params->adaptive_al_sigma_mode)) {
      new_sigma = params->al_sigma_max;
      reason = "restart_bangbang_up";
    } else if (dual_gap_dominant &&
               adaptive_sigma_mode_allows_down(
                   params->adaptive_al_sigma_mode)) {
      new_sigma = params->al_sigma_min;
      reason = "restart_bangbang_down";
    }
    break;
  default:
    break;
  ...
  return apply_new_al_sigma(state, params, new_sigma, true, reason, true);
}

bool maybe_decrease_al_sigma_on_major_iteration(
    fa_cp_solver_state_t *state, const fa_cp_parameters_t *params) {
  if (!params->adaptive_al_sigma || !state->use_al_qp) {
    return false;
  }
  ...
  if (state->al_sigma <= 1.01 * params->al_sigma_min) {
    return false;
  }

  double time_limit = params->termination_criteria.time_sec_limit;
  if (!(time_limit > 0.0) ||
      state->cumulative_time_sec < fmax(0.25 * time_limit, 120.0)) {
    return false;
  }

  double primal_small_threshold =
      fmax(1000.0 * params->termination_criteria.eps_feasible_relative, 1e-3);
  double dual_large_threshold =
      fmax(10000.0 * params->termination_criteria.eps_feasible_relative, 1e-1);
  double gap_large_threshold =
      fmax(10000.0 * params->termination_criteria.eps_optimal_relative, 1e-2);
  double dual_small_threshold =
      fmax(100.0 * params->termination_criteria.eps_feasible_relative, 1e-5);
  double gap_small_threshold =
      fmax(1000.0 * params->termination_criteria.eps_optimal_relative, 1e-3);

  bool gap_dual_stalled_switch =
      (rel_primal <= primal_small_threshold && rel_dual >= dual_large_threshold &&
       rel_gap >= gap_large_threshold);
  if (gap_dual_stalled_switch && state->al_sigma_gap_dual_switch_blocked) {
    gap_dual_stalled_switch = false;
  }
  bool primal_only_stalled_switch =
      (rel_primal >= primal_small_threshold && rel_dual <= dual_small_threshold &&
       rel_gap <= gap_small_threshold);

  if (!gap_dual_stalled_switch && !primal_only_stalled_switch) {
    return false;
  }

  const char *reason = "major_iter_tail_switch";
  if (gap_dual_stalled_switch) {
    reason = "gap_dual_stalled";
  } else if (primal_only_stalled_switch) {
    reason = "primal_only_stalled";
  }

  return apply_new_al_sigma(state, params, params->al_sigma_min, false, reason,
                            false);
}
...
void perform_restart(fa_cp_solver_state_t *state,
  ...
  compute_delta_solution_kernel<<<state->num_blocks_primal_dual,
      ...
      state->fa_cp_dual_solution, state->delta_dual_solution,
      state->num_variables, state->num_constraints);
  ...
  maybe_update_al_sigma_on_restart(state, params);

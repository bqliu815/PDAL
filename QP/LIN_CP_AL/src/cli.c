...
void save_solver_summary(const cp_al_result_t *result, const char *output_dir,
...
void print_usage(const char *prog_name) {
  ...
  fprintf(stderr, "      --no_al_qp                      "
                  "Disable the AL primal-dual update and use baseline CP_AL.\n");
  fprintf(stderr, "      --sigma_init <float>            "
                  "Initial sigma for linearized single-parameter CP-AL. "
                  "Use 0 for automatic initialization (default).\n");
  fprintf(stderr, "      --al_lambda <float>             "
                  "Deprecated alias for --sigma_init.\n");
  fprintf(stderr, "      --reflection_coefficient <float>"
                  " Reflection parameter gamma in the reflected Halpern PFBS "
                  "outer step (default: 0.0).\n");
...
int run_cp_al(int argc, char *argv[]) {
  ...
  static struct option long_options[] = {
      ...
      {"sigma_init", required_argument, 0, 1016},
      {"al_lambda", required_argument, 0, 1017},
      {"reflection_coefficient", required_argument, 0, 1018},
      {0, 0, 0, 0}};
  ...
  while ((opt = getopt_long(argc, argv, "hv:fp", long_options, NULL)) != -1) {
    ...
    case 1015: // --no_al_qp
      ...
      break;
    case 1016: // --sigma_init
      params.sigma_init = atof(optarg);
      break;
    case 1017: // --al_lambda (deprecated alias)
      params.sigma_init = atof(optarg);
      break;
    case 1018: // --reflection_coefficient
  ...
  cp_al_result_t *result = optimize(&params, problem);
...
int main(int argc, char *argv[]) { run_cp_al(argc, argv); }

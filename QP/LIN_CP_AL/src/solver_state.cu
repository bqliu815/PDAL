...
static void initialize_sparse_component_obj(cp_al_solver_state_t *state,
...
static void initialize_lowrank_component_obj(cp_al_solver_state_t *state,
                                             const qp_problem_t *problem) {
...
static void initialize_quadratic_obj_term(cp_al_solver_state_t *state,
                                          const qp_problem_t *problem) {
  ...
  if (state->quadratic_objective_term->quad_obj_type == CP_AL_NON_Q)
    return;
...
static void initialize_inner_solver(cp_al_solver_state_t *state) {
  state->inner_solver =
      (inner_solver_t *)safe_calloc(1, sizeof(inner_solver_t));
  state->inner_solver->has_inner_loop = false;
  if (!(state->quadratic_objective_term->quad_obj_type == CP_AL_NON_Q ||
        state->quadratic_objective_term->quad_obj_type == CP_AL_DIAG_Q)) {
...
static void decide_problem_type(cp_al_solver_state_t *state) {
  if (state->quadratic_objective_term->quad_obj_type == CP_AL_NON_Q)
...
void initialize_quadratic_term_information(cp_al_solver_state_t *state,
                                           const cp_al_parameters_t *params) {
  if (state->quadratic_objective_term->quad_obj_type == CP_AL_SPARSE_Q) {
  ...
  if (state->quadratic_objective_term->quad_obj_type == CP_AL_DIAG_Q) {
...
initialize_solver_state(const cp_al_parameters_t *params,
  ...
  cp_al_solver_state_t *state =
      (cp_al_solver_state_t *)safe_calloc(1, sizeof(cp_al_solver_state_t));
  ...
  state->use_al_qp = params->use_al_qp;
  ...
  if (working_problem->primal_start) {
    ...
    CUDA_CHECK(cudaMemcpy(state->current_primal_solution, rescaled, var_bytes,
                          cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(state->cp_al_primal_solution, rescaled, var_bytes,
                          cudaMemcpyHostToDevice));
    free(rescaled);
  }
  if (working_problem->dual_start) {
    ...
    CUDA_CHECK(cudaMemcpy(state->current_dual_solution, rescaled, con_bytes,
                          cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(state->cp_al_dual_solution, rescaled, con_bytes,
                          cudaMemcpyHostToDevice));
    free(rescaled);
  }
  CUDA_CHECK(cudaGetLastError());
  ...
  state->fixed_point_error = INFINITY;
  state->initial_fixed_point_error = INFINITY;
  ...
  CUSPARSE_CHECK(cusparseCreateDnVec(&state->vec_primal_sol,
                                     state->num_variables,
                                     state->cp_al_primal_solution, CUDA_R_64F));
  CUSPARSE_CHECK(cusparseCreateDnVec(&state->vec_dual_sol,
                                     state->num_constraints,
                                     state->cp_al_dual_solution, CUDA_R_64F));

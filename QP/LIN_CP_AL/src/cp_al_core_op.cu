...
halpern_update_kernel(const double *initial_primal, double *current_primal,
                      ...
                      const double *cp_al_dual, int n_vars, int n_cons,
...
halpern_update_kernel(const double *initial_primal, double *current_primal,
  ...
  if (i < n_vars) {
    double relaxed =
        (1.0 + reflection_coeff) * cp_al_primal[i] -
        reflection_coeff * current_primal[i];
    current_primal[i] = weight * relaxed + (1.0 - weight) * initial_primal[i];
  } else if (i < n_vars + n_cons) {
    int idx = i - n_vars;
    double relaxed =
        (1.0 + reflection_coeff) * cp_al_dual[idx] -
        reflection_coeff * current_dual[idx];
    current_dual[idx] = weight * relaxed + (1.0 - weight) * initial_dual[idx];
  }
}
...
static double cpal_primal_step_size(const cp_al_solver_state_t *state) {
...
static void reset_sigma_controller(cp_al_solver_state_t *state) {
  ...
  state->step_size = cpal_primal_step_size(state);
}
...
void primal_gradient_update(cp_al_solver_state_t *state, double step_size) {
  ...
  primal_gradient_descent_kernel_major<<<state->num_blocks_primal,
      ...
      state->objective_vector,
      state->quadratic_objective_term->primal_obj_product,
      state->variable_lower_bound, state->variable_upper_bound, step_size,
      state->num_variables);
}
...
void cp_al_update(cp_al_solver_state_t *state) {
  ...
  double al_sigma = 0.0;
  if (state->use_al_qp) {
    primal_step_size = cpal_primal_step_size(state);
  ...
  case CP_AL_LOW_RANK_PLUS_SPARSE_Q: {
    primal_gradient_update(state, primal_step_size);
    break;
  }
  default:
...
void halpern_update(cp_al_solver_state_t *state, double reflection_coefficient) {
  ...
  halpern_update_kernel<<<state->num_blocks_primal_dual, THREADS_PER_BLOCK>>>(
      ...
      state->cp_al_primal_solution, state->initial_dual_solution,
      state->current_dual_solution, state->cp_al_dual_solution,
...
void perform_restart(cp_al_solver_state_t *state,
  ...
  CUDA_CHECK(cudaMemcpy(state->initial_dual_solution, state->current_dual_solution,
...
void initialize_step_size_and_primal_weight(cp_al_solver_state_t *state,
  ...
  if (state->use_al_qp) {
    ...
    if (state->constraint_matrix->num_nonzeros > 0) {
      max_sv = estimate_maximum_singular_value(
          state->sparse_handle, state->blas_handle, state->constraint_matrix,
          state->constraint_matrix_t, params->sv_max_iter, params->sv_tol);
    }
    double a_sq = max_sv * max_sv;
    double l_f = fmax(state->quadratic_objective_term->norm, 0.0);
    double half_l_f = 0.5 * l_f;
    if (a_sq <= 1e-16 && half_l_f <= 1e-16) {
      state->lambda_est = 1.0;
      state->sigma_base = 1.0;
    } else {
      // For the normalized one-parameter linearized CP-AL family
      // tau^{-1} = sigma * lambda and rho = sigma, the manuscript condition
      // tau * ((sigma + rho) * ||A||^2 + L_f / 2) < 1 becomes
      // 2 * ||A||^2 / lambda + (L_f / 2) / sqrt(lambda) < 1 when sigma is at
      // its base scale sigma = 1 / sqrt(lambda). Writing t = sqrt(lambda)
      // yields t^2 - (L_f / 2) * t - 2 * ||A||^2 > 0.
      double t =
          0.5 * (half_l_f + sqrt(half_l_f * half_l_f + 8.0004 * a_sq));
      t = fmax(t, 1e-16);
      state->lambda_est = t * t;
      state->sigma_base = 1.0 / t;
    }
    state->sigma_min = state->sigma_base;
    state->sigma_max = 100.0 * state->sigma_base;
    if (params->sigma_init > 0.0) {
      state->sigma = params->sigma_init;
    } else {
      state->sigma = 6.0 * state->sigma_base;
    }
    ...
    double l_f = fmax(state->quadratic_objective_term->norm, 0.0);
    double half_l_f = 0.5 * l_f;
    double weight = fmax(state->primal_weight, 1e-16);
    if (a_sq <= 1e-16 && half_l_f <= 1e-16) {
      state->step_size = 1.0;
    } else if (a_sq <= 1e-16) {
      state->step_size = 0.99 * weight / fmax(half_l_f, 1e-16);
    } else {
      double linear_coeff = half_l_f / weight;
      double eta =
          (-linear_coeff + sqrt(linear_coeff * linear_coeff + 4.0 * a_sq)) /
          (2.0 * a_sq);
      state->step_size = 0.99 * eta;
    }
  }
}
...
void compute_fixed_point_error(cp_al_solver_state_t *state) {
  ...
  if (state->use_al_qp) {
    ...
    double tau = state->step_size / state->primal_weight;
    double rho = state->step_size * state->primal_weight;
    double fixed_point_sq = primal_norm * primal_norm / fmax(tau, 1e-16) +
                            dual_norm * dual_norm / fmax(rho, 1e-16) +
                            2.0 * cross_term;
    state->fixed_point_error = sqrt(fmax(0.0, fixed_point_sq));
  }
  ...
  if (state->inner_solver->has_inner_loop) {
    double primal_step_for_tol =
        state->use_al_qp ? cpal_primal_step_size(state)
